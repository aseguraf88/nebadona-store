...
// NUEVAS IMPORTACIONES PARA EL CSV
...
export const updateProduct = async (req, res) => {
    try {
        ...
        if (error?.code === 11000 && error?.keyPattern?.sku) {
            return res
                .status(400)
                .json({ message: 'El SKU ya existe. Debe ser unico.' })
        }

        if (error?.name === 'ValidationError' && error?.errors?.sku) {
            return res
                .status(400)
                .json({ message: error.errors.sku.message || 'SKU invalido.' })
        }
...
// ==========================================
// NUEVA FUNCIÓN: IMPORTACIÓN DE CSV
// ==========================================
export const importProductsCsv = async (req, res) => {
    try {
        ...
        stream
            ...
            .on('data', (data) => {
                const cleanData = {
                    name: data.Nombre?.trim() || data.name?.trim(),
                    product_category:
                        data.Categoría?.trim() || data.category?.trim(),
                    stock: parseInt(data.Stock || data.stock) || 0,
                    price: parseInt(data.Precio || data.price) || 0,
                    size: data.Talla?.trim() || data.size?.trim() || '',
                    color: data.Color?.trim() || data.color?.trim() || '',
                    franchise_name:
                        data.Franquicia?.trim() || data.franchise?.trim() || '',
                    character_name:
                        data.Personaje?.trim() || data.character?.trim() || '',
                    design_theme: data.Tema?.trim() || data.theme?.trim() || '',
                    status: 'DRAFT',
                }

                if (!cleanData.name || !cleanData.product_category) {
                    errors.push(
                        `Fila ignorada por falta de Nombre o Categoría: ${JSON.stringify(data)}`
                    )
                } else {
            ...
            .on('end', async () => {
                try {
                    ...
                    const insertResult = await ProductModel.insertMany(
                        results,
                        { ordered: false }
                    )

                    return res.status(200).json({
                        message: `Importación masiva completada. ${insertResult.length} productos insertados en estado DRAFT.`,
                    ...
                    if (
                        ...
                        return res.status(207).json({
                            message: `Importación finalizada con algunas colisiones. ${successCount} productos insertados. Algunos fueron ignorados por SKU duplicado.`,
                            totalImported: successCount,
                            errors: errors,
                        })
                    }
                    console.error('Error insertMany:', insertError)
                    return res
                        .status(500)
                        .json({
                            message:
                                'Error crítico al insertar en la base de datos.',
                        })
                }
            })
    } catch (error) {
